...
public class DoctorServiceImpl implements DoctorService {
	...
	public List<DoctorEO> filterDoctors(
	    ...
	    System.out.println("DEBUG: docId=" + docId + ", hosId=" + hosId + ", specialization=" + specialization);

	    if (docId != null) criteriaList.add(Criteria.where("doc_Id").is(docId));
	    if (hosId != null) criteriaList.add(Criteria.where("hos_Id").is(hosId));
	    if (specialization != null) criteriaList.add(Criteria.where("specialization").is(specialization));
	    if (availabilityStatus != null) criteriaList.add(Criteria.where("availability_status").is(availabilityStatus));
	    if (licenseNumber != null) criteriaList.add(Criteria.where("license_number").is(licenseNumber));
	    ...
	    if (rating != null) {
	        criteriaList.add(Criteria.where("rating").gte(rating));
	    }
	    ...
	    if (!criteriaList.isEmpty())
	        ...
	        return doctorRepo.findAll(); // No filters at all, return all

	    System.out.println("DEBUG: Built query: " + query);
